...
| `https://gainshin.github.io/Resume.html` | 0.8 | 學術履歷 |
...
| `meta description` | MISt、ACT Research Group、co-op 定位 |
| `canonical` | `https://gainshin.github.io/Resume.html` |
| JSON-LD | `@type: WebPage`，`about` / `author` / `isPartOf` → 主站 `#person` / `#website` |
...
4. **canonical / OG** — 勿改回無尾斜線或錯誤 LinkedIn slug。  
5. **語意標題** — 維持全頁單一 `<h1>`；新 section 補 `aria-labelledby` + heading `id`。
